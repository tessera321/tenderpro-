          ...
          <div style={{ marginTop: 12 }}>
            ...
            {progress === 40 && (
              <div style={{ marginTop: 8, fontSize: 12, color: 'var(--ink3)' }}>
                Claude анализирует структуру файла и извлекает позиции — это занимает 15–30 секунд...
              </div>
            )}
          </div>
        )}
        ...
        {result && (
          <div className="card" style={{ marginTop: 16 }}>
            <div className="card-head" style={{ background: 'var(--green-l)' }}>
              <svg width="18" height="18" fill="none" stroke="var(--green)" viewBox="0 0 24 24">
            ...
            <div className="card-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 16 }}>
                <div>
                  <div className="stat-label">Позиций найдено</div>
                  <div style={{ fontSize: 24, fontWeight: 700 }}>{result.items?.length || 0}</div>
                </div>
                <div>
                  <div className="stat-label">С ценами СМР</div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--green)' }}>
                    {result.items?.filter((i: any) => i.smr_price > 0).length || 0}
                  </div>
                </div>
                <div>
                  <div className="stat-label">Итого</div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--green)' }}>
                    {fmt(result.items?.reduce((s: number, i: any) => s + (i.total || 0), 0) || 0)}
                  </div>
                </div>
              </div>
              ...
              <div style={{ background: 'var(--bg)', borderRadius: 'var(--r)', padding: '12px', marginBottom: 14, maxHeight: 200, overflowY: 'auto', fontSize: 12 }}>
                {result.items?.slice(0, 10).map((item: any, i: number) => (
                  <div key={i} style={{ padding: '4px 0', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <span style={{ color: item.smr_price > 0 ? 'var(--green)' : 'var(--ink3)', minWidth: 80, textAlign: 'right' }}>
                      {item.smr_price > 0 ? fmt(item.smr_price) : 'нет цены'}
                    </span>
                  </div>
                ))}
                {result.items?.length > 10 && (
                  <div style={{ padding: '6px 0', color: 'var(--ink3)', textAlign: 'center' }}>
                    + ещё {result.items.length - 10} позиций
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                {result.tender_id && (
                  <button
                    ...
                    onClick={() => router.push(`/dashboard/tenders/${result.tender_id}`)}
                  >
